		...
		<div className="Attempt-List-Main">
			...
			<ul className="Attempt-List">
				{props.attempts.filter(attempt => {
					return search === '' ? attempt : attempt.login.includes(search) || attempt.password.includes(search)
				}).map(attempt => <LoginAttempt>{attempt.login} {attempt.password}</LoginAttempt>)}
			</ul>
		</div>)
};
